...
global $theme_key;
if (db_is_active()) {
  stanford_c1_initialize_theme_settings($theme_key);
}
 ...
 */
function stanford_c1_preprocess_maintenance_page(&$vars) {
  if (db_is_active()) {
    stanford_c1_preprocess_page($vars);
  }
}
 ...
 * Page preprocessing
 */
function stanford_c1_preprocess_page(&$vars) {
  ...
  $themes = stanford_c1_theme_paths($theme_key);
 ...
 //hook into color module
  ...
  if (theme_get_setting('fix_css_limit') && !variable_get('preprocess_css', FALSE)) {
    ...
    if (stanford_c1_css_count($css) > (30 - $style_count)) {
      $styles = '';
      $suffix = "\n".'</style>'."\n";
 ...
 * Node preprocessing
 */
function stanford_c1_preprocess_node(&$vars) {
 ...
 */
function stanford_c1_preprocess_comment(&$vars) {
  global $user;
 ...
 */
function stanford_c1_preprocess_comment_wrapper(&$vars) {
 ...
 * Block preprocessing
 */
function stanford_c1_preprocess_block(&$vars) {
  ...
  if (!isset($regions)) {
    ...
    $regions = stanford_c1_set_regions($grid_width, $sidebar_first_width, $sidebar_last_width);
  }
  ...
  if (user_access('administer blocks', $user) && theme_get_setting('block_config_link')) {
    $vars['edit_links'] = '<div class="stanford_c1-edit">'. implode(' ', stanford_c1_edit_links($vars['block'])) .'</div>';
  }
}
 ...
 */
function stanford_c1_preprocess_views_view(&$vars) {
 ...
 */
function stanford_c1_preprocess_search_result(&$vars) {
 ...
 */
function stanford_c1_username($object) {
 ...
 */
function stanford_c1_file($element) {
 ...
 */
function stanford_c1_grid_row($element, $name, $class='', $width='', $extra='') {
  $output = '';
  $extra = ($extra) ? ' ' . $extra : '';
  if ($element) {
    if ($class == 'full-width') {
...
function stanford_c1_grid_block($element, $name) {
  $output = '';
  if ($element) {
 ...
 */
function stanford_c1_set_regions($grid_width, $sidebar_first_width, $sidebar_last_width) {
 ...
 */
function stanford_c1_edit_links($block) {
  ...
  if ($block->module == 'block') {
    ...
    $attributes = array('title' => t('edit the content of this Custom block (in @region)', $block_info), 'class' => 'stanford_c1-block-edit');
  }
  else {
    $text = t('configure block');
    $block_info = array('@type' => ucwords($block->module), '@region' => str_replace('_', ' ', $block->region));
    $attributes = array('title' => t('configure this @type block (in @region)', $block_info), 'class' => 'stanford_c1-block-config');
  }
  ...
  if (user_access('administer menu') && ($block->module == 'menu' || ($block->module == 'user' && $block->delta == 1))) {
    ...
    $attributes = array('title' => t('edit the menu of this @type block (in @region)', $block_info), 'class' => 'stanford_c1-edit-menu');
    $edit_links[] = l($text, $path, array('attributes' => $attributes, 'query' => $return));
  }
  return $edit_links;
}
 ...
 */
function stanford_c1_css_count($array) {
  $count = 0;
  foreach ($array as $item) {
    $count = (is_array($item)) ? $count + stanford_c1_css_count($item) : $count + 1;
  }
  return $count;
}
 ...
 */
function stanford_c1_theme_paths($theme) {
  $all_parents = array();
  $themes = list_themes();
 ...
 */
function stanford_c1_themesettings_link($prefix, $suffix, $text, $path, $options) {
  return $prefix . (($text) ? l($text, $path, $options) : '') . $suffix;
}


function stanford_c1_preprocess_search_theme_form(&$vars, $hook) {
...
function stanford_c1_process_page(&$vars) {
  // Hook into color.module
  if (module_exists('color')) {
    _color_page_alter($vars);
  }
